...
class StarryNet():
    ...
    def __init__(self,
        ...
        self.route_src = []
        self.route_time = []
        self.neighbors_time = []  # 记录邻居关系的时间点列表
        self.neighbors_recording_time = []
        self.routing_table_recording_interval = 1  # 默认每秒记录一次
        self.routing_table_recording_enabled = False  # 是否启用路由表记录
    ...
    def run_routing_deamon(self):
        routing_thread = sn_Routing_Init_Thread(
            self.remote_ssh, self.remote_ftp, self.orbit_number,
            self.sat_number, self.constellation_size, self.fac_num,
            self.file_path, self.configuration_file_path, self.sat_bandwidth, self.sat_ground_bandwidth,
    ...
    def set_neighbors_recording(self, time_index):
        """
        设置在指定时间记录所有节点的邻居关系
        :param time_index: 记录邻居关系的时间点
        """
        self.neighbors_recording_time.append(time_index)
    
    def set_routing_table_recording(self, enabled=True, interval=1):
        """
        设置路由表记录配置
        :param enabled: 是否启用路由表记录
        :param interval: 记录间隔（秒），默认为1秒
        """
        self.routing_table_recording_enabled = enabled
        self.routing_table_recording_interval = interval

        
    def set_neighbors_log(self, time_index):
        """
        记录需要获取邻居关系的时间点
        :param time_index: 时间点索引
        """
        self.neighbors_time.append(time_index)
    ...
    def start_emulation(self):
        ...
        sn_thread = sn_Emulation_Start_Thread(
            ...
            self.neighbors_recording_time, self.routing_table_recording_enabled,
            self.routing_table_recording_interval)
        sn_thread.start()
        sn_thread.join()
